...
#SBATCH --job-name=ebt_25m_512
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-gpu=24
#SBATCH --mem-per-cpu=15G
#SBATCH --time=72:00:00
#SBATCH --output=output/logs/ebt_25m_512_%A_%a.out
#SBATCH --error=output/logs/ebt_25m_512_%A_%a.err
#SBATCH --array=0-2
...
# Model size sweep: same as MDM (306M, 425M, 489M)
source utils/model_params.sh
case $SLURM_ARRAY_TASK_ID in
    0) MODEL_PARAM=("${PARAM_306M[@]}") ; SIZE=306M ;;
    1) MODEL_PARAM=("${PARAM_425M[@]}") ; SIZE=425M ;;
    2) MODEL_PARAM=("${PARAM_489M[@]}") ; SIZE=489M ;;
esac
...
VARIANT=ebt_25m_512_${SIZE}_500ep
...
GPT_ARGS=" \
    ...
    $EBT_ARGS \
    "
# To use hybrid bf16 mode instead (faster, less memory):
# Replace --ebt-fp32 with --bf16, update DS config bf16.enabled=true,
# and increase MICRO_BATCH_SIZE
